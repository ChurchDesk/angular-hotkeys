...
(function() {
  ...
  angular.module('cfp.hotkeys', []).provider('hotkeys', function($injector) {
    ...
    /**
     * Configurable setting to set the number of columns in the cheat sheet
     * @type {Number}
     */
    this.cheatSheetColumnsCount = 2;
    ...
    this.template = '<div class="cfp-hotkeys-container fade" ng-class="{in: helpVisible}" style="display: none;"><div class="cfp-hotkeys">' +
                      '<h4 class="cfp-hotkeys-title" ng-if="!header">{{ title | translate }}</h4>' +
                      ...
                      '<table><tbody>' +
                        '<tr ng-repeat="row in cheatSheetColumns">' +
                          ...
                          '</td>' +
                          '<td class="cfp-hotkeys-text" ng-repeat-end>{{ hotkey.description | translate }}</td>' +
                        '</tr>' +
                      '</tbody></table>' +
    ...
    this.$get = function ($rootElement, $rootScope, $compile, $window, $document) {
      ...
      function refreshColumns() {
        ...
        var numRows = Math.ceil(scope.hotkeys.length / (scope.hotkeys.length > 8 ? self.cheatSheetColumnsCount : 1));

        for (var i = 0; i < numRows; i++) {
          var row = [];
          var numColumns = scope.hotkeys.length > 8 ? self.cheatSheetColumnsCount : 1;
